...
SimpleCenterGradientAnim::SimpleCenterGradientAnim () {
    AbstractTubeAnimation::AbstractTubeAnimation();	
}

void SimpleCenterGradientAnim::init(string name, vector<ofxTube*> * tubes) {
	AbstractTubeAnimation::init(name, tubes);
}

void SimpleCenterGradientAnim::launch() {
...
void SimpleCenterGradientAnim::onAnimationStart(ofxTubeEvent * args) {
...
void SimpleCenterGradientAnim::onAnimationEnd(ofxTubeEvent * args) {
    AbstractTubeAnimation::onAnimationEnd(args);
	ofxTube * tube = tubes->at(args->id);
	
}

void SimpleCenterGradientAnim::onAnimationLoopEvent(int & a) {
	
}

void SimpleCenterGradientAnim::update () {
	...
	for ( int i = 0; i < (int)tubes->size(); i++ ) {
		...
		int middle = 56;
		int limit = (int)(tube->dumbPct * middle*4);
		...
		for ( int i= 0; i<AbstractTubeAnimation::numOfTubePixels; i++ ) {
			...
			if ( i<middle ) {
				...
				if ( i> middle - limit ) {
					...
					tube->setPixelAlpha(i, 1.0 - ((float)index / (float)limit), 0.0); 
				} else {
...
void SimpleCenterGradientAnim::quit() { 
